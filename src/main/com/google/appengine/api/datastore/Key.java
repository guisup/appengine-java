...
public final class Key implements Serializable, Comparable<Key> {
  ...
  Key(String kind, Key parentKey, String name) {
    this(kind, parentKey, NOT_ASSIGNED, name, (AppIdNamespace) null);
  }
  ...
  public boolean equals(Object object) {
    if (object instanceof Key) {
      ...
      if (name == null && id == NOT_ASSIGNED && key.id == NOT_ASSIGNED) {
        return false;
      }
      if (id != key.id ||
          !kind.equals(key.kind) ||
          (name != null && !name.equals(key.name))) {
        return false;
      }
      if (parentKey != key.parentKey && (parentKey == null ||
          !parentKey.equals(key.parentKey))) {
        return false;
      }
      return true;
    }
